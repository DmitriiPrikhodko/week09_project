...
with col1:
    uploaded_files_local = st.file_uploader(
        "Загрузите изображения", type=["jpg", "png", "jpeg"], accept_multiple_files=True
    )
...
def blur_detections(img, results):  # type: ignore
    ...
    )  # Получить numpy массив координат боксов
...
fig1.add_trace(
    go.Bar(
        ...
        y=[
            last_epoch["metrics/precision(B)"],
...
fig1.update_layout(title="Основные метрики", yaxis=dict(range=[0, 1]), showlegend=False)
